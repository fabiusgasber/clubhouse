...
const isCancelMembership = (value) => {
    if(value !== process.env.CANCEL_MEMBER) throw new Error("Wrong answer. Try again.");
    return true;
};

const isCancelAdmin = (value) => {
    if(value !== process.env.CANCEL_ADMIN) throw new Error("Wrong answer. Try again.");
    return true;
};

const isDeleteAccount = (value) => {
    if(value !== process.env.DELETE_ACCOUNT) throw new Error("Wrong answer. Try again.");
    return true;
}
